...
Only a new low-level MCP action client session applies `profile_source`, identified exactly as a JSON-RPC `initialize` POST without `mcp-session-id`. Follow-up requests carrying the session header and non-initialization requests do not reset the target. An explicit reset acquires source and target locks in sorted physical-name order, stops the target backend process group, atomically replaces the target from the source, starts the new backend generation, and delivers the triggering initialize request through that generation before releasing the locks. Response-body streaming then continues outside the lifecycle locks. Operations on different target profiles retain independent locks and may proceed in parallel.
