...
Gateway readiness uses `browser-vpn-runtime-vpn-egress-healthcheck` with one platform-selected stable external TCP target. The command negotiates with the local Dante listener and completes a SOCKS5 `CONNECT` to that target. A TCP accept on port `1080` alone is deliberately insufficient because Dante retains its listener while the down hook pauses the process. Since Dante target traffic is firewall-constrained to `tun0`, a successful CONNECT proves current tunnel egress and gateway-side target DNS; the command never opens the target directly as root.
