...
def test_kubernetes_writeback_job_snapshots_stopped_runtime_profile_to_writable_target() -> None:
    """Provide one executable profile writeback boundary outside the long-lived browser pod."""
    job = yaml.safe_load(Path("deploy/k8s/profile-writeback-job.yaml").read_text(encoding="utf-8"))
    ...
    volume_by_name_map = {volume["name"]: volume for volume in pod_spec["volumes"]}
    assert volume_by_name_map["profile-runtime"]["persistentVolumeClaim"] == {
        "claimName": "browser-vpn-runtime-profile-runtime",
        "readOnly": True,
    }
    assert volume_by_name_map["profile-writeback"]["persistentVolumeClaim"]["claimName"] == (
        "browser-vpn-runtime-profile-writeback"
    )
    mount_by_path_map = {mount["mountPath"]: mount for mount in container["volumeMounts"]}
    assert mount_by_path_map["/runtime-profile"]["readOnly"] is True
    assert mount_by_path_map["/writeback"]["name"] == "profile-writeback"
    assert mount_by_path_map["/writeback"].get("readOnly", False) is False
